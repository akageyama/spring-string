...
class SpringElement
{
                    ...
                    float[] posz)
  {
    ...
    if ( distance >= SPRING_ELONGATION_CUT_LIMIT )
      springConst = 0.0;

    float pullForceAmplitude = springConst * (distance - SPRING_NATURAL_LENGTH);
               ...
               float[] posz)
  {
    ...
    float distance = dist(ax, ay, az, bx, by, bz);
    float elongation = distance - SPRING_NATURAL_LENGTH;
